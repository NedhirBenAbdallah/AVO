...
const EfficienceChartWidget = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Update chart data
        updateChartData(tracksData, lignesData);
      } catch (error) {
  ...
  filteredData.sort((a, b) => new Date(a.date_start) - new Date(b.date_start));
    ...
    updateChartData(filteredData, lignes);
  }, [selectedLigne, selectedWeek, tracks, lignes]);

  const updateChartData = (data, lignes) => {
    const labels = data.map(track => {
      const date = new Date(track.date_start);
      return date.toLocaleDateString();
    });

    const efficienceValues = data.map(track => {
      const ligne = lignes.find(l => l.id === track.ligne_id);
      const HProduit = (track.qt * (ligne ? ligne.h1000 : 0)) / 1000;
      return track.h_paye !== 0 ? (HProduit / track.h_paye) * 100 : 0;
    });
...
export default EfficienceChartWidget;