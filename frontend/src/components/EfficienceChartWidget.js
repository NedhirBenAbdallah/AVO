...
const EfficiencyChartWidget = () => {
  ...
  const calculateEfficiency = (track, lignes) => {
  ...
  const groupByYear = (tracks) => {
    ...
    tracks.forEach(track => {
      ...
      yearGroups[year].total += calculateEfficiency(track, lignes);
  ...
  const groupByMonth = (tracks, year) => {
    ...
    tracks.forEach(track => {
      ...
      if (date.getFullYear() === year) {
        ...
        monthGroups[month].total += calculateEfficiency(track, lignes);
  ...
  const groupByWeek = (tracks, year, month) => {
    ...
    tracks.forEach(track => {
      ...
      if (date.getFullYear() === year && date.getMonth() === new Date(Date.parse(month + " 1")).getMonth()) {
        ...
        weekGroups[week].total += calculateEfficiency(track, lignes);
  ...
  const groupByDay = (tracks, year, month, week) => {
    return tracks
      ...
      .map(track => ({
        ...
        efficiency: calculateEfficiency(track, lignes)
      }));
  };
